...
def to_metric_coordinates(data, field_dimen=(106., 68.)):
    ...
    """
    # Assurez-vous de ne sélectionner que les colonnes de position
    x_columns = [c for c in data.columns if c.endswith('_x') and 'Time' not in c]
    y_columns = [c for c in data.columns if c.endswith('_y') and 'Time' not in c]
    
    # Appliquer la transformation pour convertir en mètres
